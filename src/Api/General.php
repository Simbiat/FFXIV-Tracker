    ...
    protected function genData(array $path): array
    {
        ...
        try {
            ...
            if ($this->name_for_links === 'achievement') {
                $data = match ($path[1]) {
                    'update' => new \Simbiat\FFXIV\Entities\Achievement($path[0])->updateFromApi(),
                    'lodestone' => new \Simbiat\FFXIV\Entities\Achievement($path[0])->getFromLodestone(),
                    default => new \Simbiat\FFXIV\Entities\Achievement($path[0])->getArray(),
                };
            } else {
                $data = match ($path[1]) {
